...
if(!empty($direction) && $direction == "show"){
	...
	$query_str	= "SELECT * FROM ".$tpref."products WHERE ID_PRODUCT IS NOT NULL ".@$condition." ORDER BY ID_PRODUCT DESC ";
...
if(!empty($direction) && $direction == "edit"){
	if(!empty($no)){
		$q_produk_edit 	= $db->query("SELECT * FROM ".$tpref."products WHERE ID_PRODUCT = '".$no."' ORDER BY ID_PRODUCT DESC");
		...
		@$discount		= $dt_produk_edit->DISCOUNT;
